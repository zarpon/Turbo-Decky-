...
# O perfil persistente deve criar um helper executável e uma unidade ordenada
# depois de swap.target; o perfil ZRAM deve removê-los.
...
# Verificação runtime bem-sucedida em sysfs simulado.
ROOTFS=""
DRY_RUN=0
...
# Reproduz uma escrita que retorna sucesso sem alterar enabled. A função precisa
# detectar a pós-condição falsa e falhar, em vez de informar perfil concluído.
printf 'N\n' > "$ZSWAP_SYSFS_DIR/enabled"
if (
  write_runtime_value() {
...
# As funções finais precisam gerir o serviço sem chamar reversão durante troca
# de perfil. A reversão explícita deve removê-lo antes de restaurar arquivos.
