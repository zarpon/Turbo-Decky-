...
versao="3.2 - 31-03 r5 - Timeless Child"
autor="Jorge Luis"
...
optimize_zram() {
    # ZRAM SteamOS-safe + recompressão automática (idle 3min + huge + huge_idle)
...
aplicar_zswap() {
    ...
    local kernel_params=("zswap.enabled=1" "zswap.compressor=lz4" "zswap.max_pool_percent=35" "zswap.zpool=zsmalloc" "zswap.shrinker_enabled=0" "mitigations=off" "audit=0" "nmi_watchdog=0" "nowatchdog" "split_lock_detect=off" "amd_pstate=active" "amdgpu.ppfeaturemask=0xfffd7fff" "preempt=full" "threadirqs")
...
aplicar_zram() {
    ...
    local kernel_params=("zswap.enabled=0" "mitigations=off" "audit=0" "nmi_watchdog=0" "nowatchdog" "split_lock_detect=off" "amd_pstate=active" "amdgpu.ppfeaturemask=0xfffd7fff" "preempt=full" "threadirqs")
