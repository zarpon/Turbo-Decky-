...
versao="1.7.2. ENDLESS GAME" autor="Jorge Luis" pix_doacao="jorgezarpon@msn.com"

--- constantes e variáveis ---

readonly swapfile_path="/home/swapfile" readonly grub_config="/etc/default/grub" readonly zswap_swapfile_size_gb="8" readonly zram_swapfile_size_gb="2" readonly backup_suffix="bak-turbodecky" readonly logfile="/var/log/turbodecky.log"

--- Constantes para otimização do MicroSD ---

readonly sdcard_device="/dev/mmcblk0p1" readonly nvme_shadercache_target_path="/home/deck/sd_shadercache"
...
readonly base_sysctl_params=( "vm.swappiness=100" "vm.vfs_cache_pressure=66" "vm.dirty_background_bytes=262144000" "vm.dirty_bytes=524288000" "vm.dirty_expire_centisecs=1500" "vm.dirty_writeback_centisecs=1500" "vm.min_free_kbytes=131072" "vm.page-cluster=0" "vm.compaction_proactiveness=10" "kernel.numa_balancing=0" "vm.watermark_scale_factor=125" "vm.stat_interval=15" "vm.compact_unevictable_allowed=0" "vm.watermark_boost_factor=0" "vm.zone_reclaim_mode=0" "vm.max_map_count=2147483642" "vm.mmap_rnd_compat_bits=16" "vm.unprivileged_userfaultfd=1" "vm.hugetlb_optimize_vmemmap=0" "fs.aio-max-nr=1048576" "fs.epoll.max_user_watches=100000" "fs.inotify.max_user_watches=524288" "fs.pipe-max-size=2097152" "fs.pipe-user-pages-soft=65536" "fs.file-max=1000000" "kernel.nmi_watchdog=0" "kernel.soft_watchdog=0" "kernel.watchdog=0" "kernel.core_pattern=/dev/null" "kernel.core_pipe_limit=0" "kernel.printk_devkmsg=off" "net.core.default_qdisc=fq_pie" "net.ipv4.tcp_congestion_control=bbr" "net.core.netdev_max_backlog=16384" )

--- listas de serviços para ativar/monitorar ---

readonly otimization_services=( "thp-config.service" "io-boost.service" "hugepages.service" "ksm-config.service" "kernel-tweaks.service" )

readonly unnecessary_services=( "gpu-trace.service" "steamos-log-submitter.service" "cups.service" )

--- variáveis de ambiente (Configuração de Jogos) ---

Nota: DXVK_STATE_CACHE_PATH usa a variável definida acima

readonly game_env_vars=( "RADV_PERFTEST=gpl,aco,sam,shader_ballot" "RADV_DEBUG=novrsflatshading" "RADEONSI_SHADER_PRECOMPILE=true"

"MESA_DISK_CACHE_COMPRESSION=zstd" "MESA_SHADER_CACHE_MAX_SIZE=6G" "VKD3D_SHADER_CACHE=1"

"PROTON_FORCE_LARGE_ADDRESS_AWARE=1" "WINE_DISABLE_PROTOCOL_FORK=1" "WINE_DISABLE_WRITE_WATCH=1"
...
_ui_info() { echo -e "\n[info] $1: $2"; # Se tiver zenity e for erro ou sucesso final, exibe popup if command -v zenity &>/dev/null; then if [[ "$1" == "erro" ]]; then zenity --error --text="$2" --width=300 2>/dev/null || true elif [[ "$1" == "sucesso" ]]; then # Não bloqueia execução com sucesso para não irritar, apenas notifica zenity --notification --text="$2" 2>/dev/null || true fi fi }

_log() { mkdir -p "$(dirname "$logfile")" 2>/dev/null || true touch "$logfile" 2>/dev/null || true echo "$(date '+%F %T') - $*" | tee -a "$logfile" }

if [[ $EUID -ne 0 ]]; then if command -v zenity &>/dev/null; then zenity --error --text="Este script deve ser executado como root (sudo)." --width=300 fi echo "❌ erro: este script deve ser executado como root (sudo)." >&2; exit 1; fi

steamos_readonly_cmd="" if command -v steamos-readonly &>/dev/null; then steamos_readonly_cmd=$(command -v steamos-readonly) fi

_backup_file_once() { local f="$1"; local backup_path="${f}.${backup_suffix}" if [[ -f "$f" && ! -f "$backup_path" ]]; then cp -a --preserve=timestamps "$f" "$backup_path" 2>/dev/null || cp -a "$f" "$backup_path" _log "backup criado: $backup_path" fi }

_restore_file() { local f="$1"; local backup_path="${f}.${backup_suffix}" if [[ -f "$backup_path" ]]; then mv "$backup_path" "$f" _log "arquivo '$f' restaurado a partir de $backup_path" else _log "backup para '$f' não encontrado." return 1 fi }

_write_sysctl_file() { local file_path="$1"; shift; local params=("$@"); local tmp="${file_path}.tmp" if [ ${#params[@]} -eq 0 ]; then _log "erro: sem parâmetros."; return 1; fi touch "$tmp" if [[ -f "$file_path" ]]; then grep -vE '^(#.*|vm.|kernel.|fs.|net.)' "$file_path" >"$tmp" 2>/dev/null || true; fi printf "%s\n" "${params[@]}" >>"$tmp" mv "$tmp" "$file_path" sync _log "sysctl escrito: $file_path" }

_steamos_readonly_disable_if_needed() { if [[ -n "$steamos_readonly_cmd" ]]; then if "$steamos_readonly_cmd" status 2>/dev/null | grep -qi "enabled"; then "$steamos_readonly_cmd" disable || true trap 'if [[ -n "$steamos_readonly_cmd" ]]; then "$steamos_readonly_cmd" enable || true; fi' EXIT _log "steamos-readonly desativado temporariamente" else _log "steamos-readonly já estava desativado"; trap 'true' EXIT fi else trap 'true' EXIT fi }

--- NOVA FUNÇÃO: Configurar pasta DXVK ---

_setup_dxvk_folder() { _log "Configurando pasta DXVK Cache..." if [ ! -d "$dxvk_cache_path" ]; then mkdir -p "$dxvk_cache_path" _log "Pasta criada: $dxvk_cache_path" fi # Corrige permissões para o usuário 'deck' (UID 1000) # Isso é crucial pois a pasta foi criada via sudo (root) chown -R 1000:1000 "$dxvk_cache_path" 2>/dev/null || chown -R deck:deck "$dxvk_cache_path" 2>/dev/null || true chmod 755 "$dxvk_cache_path" _log "Permissões da pasta DXVK ajustadas para usuário deck." }

_optimize_gpu() { _log "aplicando otimizações amdgpu..." mkdir -p /etc/modprobe.d # Cria o arquivo que será removido na reversão echo "options amdgpu moverate=128" > /etc/modprobe.d/99-amdgpu-tuning.conf _log "arquivo /etc/modprobe.d/99-amdgpu-tuning.conf criado." }

_configure_ulimits() { _log "aplicando limite de arquivo aberto (ulimit) alto (1048576)" mkdir -p /etc/security/limits.d cat <<'EOF' > /etc/security/limits.d/99-game-limits.conf
...
soft memlock 2147483648 EOF _log "/etc/security/limits.d/99-game-limits.conf criado/atualizado." }


_configure_irqbalance() { _log "configurando irqbalance..." mkdir -p /etc/default _backup_file_once "/etc/default/irqbalance"
...
} create_persistent_configs() { _log "criando arquivos de configuração persistentes" mkdir -p /etc/tmpfiles.d /etc/modprobe.d /etc/modules-load.d

# MGLRU
cat << EOF > /etc/tmpfiles.d/mglru.conf

w /sys/kernel/mm/lru_gen/enabled - - - - 7 w /sys/kernel/mm/lru_gen/min_ttl_ms - - - - 0 EOF # THP Shrinker cat << EOF > /etc/tmpfiles.d/thp_shrinker.conf w! /sys/kernel/mm/transparent_hugepage/khugepaged/max_ptes_none - - - - 409 EOF
...
manage_unnecessary_services() { local action="$1" if [[ "$action" == "disable" ]]; then systemctl stop "${unnecessary_services[@]}" --quiet || true systemctl mask "${unnecessary_services[@]}" --quiet || true elif [[ "$action" == "enable" ]]; then systemctl unmask "${unnecessary_services[@]}" --quiet || true fi }

--- FUNÇÃO: REGRAS DE ENERGIA (HÍBRIDO AC/BATERIA COM ANTI-STUTTER) ---

create_power_rules() { _log "configurando regras dinâmicas de energia (AC/Bateria) com detecção híbrida..."
...
get_ac_state_sysfs() { for ps in /sys/class/power_supply/*; do [ -d "$ps" ] || continue local type_file="$ps/type" local online_file="$ps/online"
...
} get_ac_state_upower() { if ! command -v upower &>/dev/null; then echo "unknown" return fi local lp; lp=$(upower -e 2>/dev/null | grep -i line_power | head -n1) if [[ -z "$lp" ]]; then echo "unknown" return fi upower -i "$lp" 2>/dev/null | awk '/online/ {print $2}' | tr '[:upper:]' '[:lower:]' | tr -d ' \t\n' } is_on_ac() { local sysfs_state; sysfs_state=$(get_ac_state_sysfs) if [[ "$sysfs_state" == "1" ]]; then return 0; fi if [[ "$sysfs_state" == "0" ]]; then return 1; fi local up_state; up_state=$(get_ac_state_upower) if [[ "$up_state" =~ ^(yes|on|1)$ ]]; then return 0; fi return 1 }

if is_on_ac; then # --- MODO TOMADA (PERFORMANCE & BAIXA LATÊNCIA) --- logger "TurboDecky: Conectado a energia - Boost Seguro (Híbrido)"
...
else # --- MODO BATERIA (PADRÃO/ECONOMIA) --- logger "TurboDecky: Bateria - Revertendo (Híbrido)"
...
fi EOF chmod +x /usr/local/bin/turbodecky-power-monitor.sh

# 4. CRIAÇÃO DO SERVICE (Para ser acionado pelo UDEV/SYSTEMD)
cat <<UNIT > /etc/systemd/system/turbodecky-power-monitor.service

[Unit] Description=TurboDecky Power Monitor (oneshot udev-triggered) Wants=sys-devices-virtual-power_supply-*/device [Service] Type=oneshot ExecStart=/usr/local/bin/turbodecky-power-monitor.sh UNIT systemctl daemon-reload || true
...
SUBSYSTEM=="power_supply", ACTION=="change", ATTR{type}=="Mains", TAG+="systemd", ENV{SYSTEMD_WANTS}="turbodecky-power-monitor.service" SUBSYSTEM=="power_supply", ACTION=="change", ATTR{type}=="ACAD", TAG+="systemd", ENV{SYSTEMD_WANTS}="turbodecky-power-monitor.service" SUBSYSTEM=="power_supply", ACTION=="change", ATTR{type}=="USB_C", TAG+="systemd", ENV{SYSTEMD_WANTS}="turbodecky-power-monitor.service" SUBSYSTEM=="power_supply", ACTION=="change", ATTR{type}=="USB-PD", TAG+="systemd", ENV{SYSTEMD_WANTS}="turbodecky-power-monitor.service" UDEV
...
create_common_scripts_and_services() { _log "criando scripts e services comuns" mkdir -p /usr/local/bin /etc/systemd/system /etc/environment.d
...
#!/usr/bin/env bash sleep 5 for dev_path in /sys/block/sd* /sys/block/mmcblk* /sys/block/nvmen /sys/block/zram*; do [ -d "$dev_path" ] || continue dev_name=$(basename "$dev_path") queue_path="$dev_path/queue" echo 0 > "$queue_path/iostats" 2>/dev/null || true echo 0 > "$queue_path/add_random" 2>/dev/null || true
...
#!/usr/bin/env bash echo "always" > /sys/kernel/mm/transparent_hugepage/enabled 2>/dev/null || true echo "defer+madvise" > /sys/kernel/mm/transparent_hugepage/defrag 2>/dev/null || true echo "advise" > /sys/kernel/mm/transparent_hugepage/shmem_enabled 2>/dev/null || true echo 1 > /sys/kernel/mm/transparent_hugepage/khugepaged/defrag 2>/dev/null || true
...
echo 2048 > /sys/kernel/mm/transparent_hugepage/khugepaged/pages_to_scan 2>/dev/null || true echo 5000 > /sys/kernel/mm/transparent_hugepage/khugepaged/scan_sleep_millisecs 2>/dev/null || true echo 128 > /sys/kernel/mm/transparent_hugepage/khugepaged/max_ptes_swap 2>/dev/null || true THP chmod +x /usr/local/bin/thp-config.sh

# --- 4. SCRIPT HUGEPAGES ---
cat <<'HPS' > /usr/local/bin/hugepages.sh

#!/usr/bin/env bash echo 0 > /sys/kernel/mm/hugepages/hugepages-2048kB/nr_hugepages 2>/dev/null || true HPS chmod +x /usr/local/bin/hugepages.sh

# --- 5. SCRIPT KSM ---
cat <<'KSM' > /usr/local/bin/ksm-config.sh

#!/usr/bin/env bash echo 0 > /sys/kernel/mm/ksm/run 2>/dev/null || true KSM chmod +x /usr/local/bin/ksm-config.sh

# --- 6. SCRIPT KERNEL TWEAKS ---
cat <<'KRT' > /usr/local/bin/kernel-tweaks.sh

#!/usr/bin/env bash echo 1 > /sys/module/multi_queue/parameters/multi_queue_alloc 2>/dev/null || true echo 1 > /sys/module/multi_queue/parameters/multi_queue_reclaim 2>/dev/null || true if [ -w /sys/module/rcu/parameters/rcu_normal_after_boot ]; then echo 0 > /sys/module/rcu/parameters/rcu_normal_after_boot 2>/dev/null || true fi KRT chmod +x /usr/local/bin/kernel-tweaks.sh
...
[Unit] Description=TurboDecky ${service_name} persistence After=local-fs.target [Service] Type=oneshot ExecStart=/usr/local/bin/${service_name}.sh RemainAfterExit=true [Install] WantedBy=multi-user.target UNIT done systemctl daemon-reload || true }

otimizar_sdcard_cache() { _log "otimizando microsd..." local sdcard_mount_point; sdcard_mount_point=$(findmnt -n -o TARGET "$sdcard_device" 2>/dev/null || echo "") if [[ -z "$sdcard_mount_point" ]]; then _ui_info "erro" "microsd não montado"; return 1; fi local sdcard_steamapps_path="${sdcard_mount_point}/steamapps" local sdcard_shadercache_path="${sdcard_steamapps_path}/shadercache" if ! [ -d "$sdcard_steamapps_path" ]; then _ui_info "erro" "steamapps não encontrado"; return 1; fi if [ -L "$sdcard_shadercache_path" ]; then _ui_info "info" "já otimizado"; return 0; fi mkdir -p "$nvme_shadercache_target_path" local deck_user; deck_user=$(stat -c '%U' /home/deck 2>/dev/null || echo "deck") local deck_group; deck_group=$(stat -c '%G' /home/deck 2>/dev/null || echo "deck") chown "${deck_user}:${deck_group}" "$nvme_shadercache_target_path" 2>/dev/null || true if [ -d "$sdcard_shadercache_path" ]; then if command -v rsync &>/dev/null; then rsync -a --remove-source-files "$sdcard_shadercache_path"/ "$nvme_shadercache_target_path"/ 2>/dev/null || true find "$sdcard_shadercache_path" -type d -empty -delete 2>/dev/null || true else mv "$sdcard_shadercache_path"/* "$nvme_shadercache_target_path"/ 2>/dev/null || true rmdir "$sdcard_shadercache_path" 2>/dev/null || true fi fi ln -s "$nvme_shadercache_target_path" "$sdcard_shadercache_path" _ui_info "sucesso" "otimização do microsd concluída!" }

reverter_sdcard_cache() { _log "revertendo microsd..." local sdcard_mount_point; sdcard_mount_point=$(findmnt -n -o TARGET "$sdcard_device" 2>/dev/null || echo "") if [[ -z "$sdcard_mount_point" ]]; then _ui_info "erro" "microsd não montado"; return 1; fi local sdcard_steamapps_path="${sdcard_mount_point}/steamapps" local sdcard_shadercache_path="${sdcard_steamapps_path}/shadercache" if ! [ -L "$sdcard_shadercache_path" ]; then _ui_info "erro" "link não encontrado"; return 1; fi rm "$sdcard_shadercache_path" mkdir -p "$sdcard_shadercache_path" if command -v rsync &>/dev/null; then rsync -a --remove-source-files "$nvme_shadercache_target_path"/ "$sdcard_shadercache_path"/ 2>/dev/null || true find "$nvme_shadercache_target_path" -type d -empty -delete 2>/dev/null || true else mv "$nvme_shadercache_target_path"/* "$sdcard_shadercache_path"/ 2>/dev/null || true rmdir "$nvme_shadercache_target_path" 2>/dev/null || true fi _ui_info "sucesso" "reversão microsd concluída." }
...
_executar_reversao() { _steamos_readonly_disable_if_needed; _log "executando reversão geral"
...
aplicar_zswap() { _log "Aplicando ZSWAP (Híbrido AC/Battery)"
...
#!/usr/bin/env bash echo 1 > /sys/module/zswap/parameters/enabled 2>/dev/null || true echo zstd > /sys/module/zswap/parameters/compressor 2>/dev/null || true echo 35 > /sys/module/zswap/parameters/max_pool_percent 2>/dev/null || true echo zsmalloc > /sys/module/zswap/parameters/zpool 2>/dev/null || true echo 1 > /sys/module/zswap/parameters/shrinker_enabled 2>/dev/null || true echo 1 > /sys/kernel/mm/page_idle/enable 2>/dev/null || true sysctl -w vm.fault_around_bytes=32 2>/dev/null || true ZSWAP_SCRIPT chmod +x /usr/local/bin/zswap-config.sh

cat <<UNIT > /etc/systemd/system/zswap-config.service

[Unit] Description=Configuracao ZSWAP Persistent After=local-fs.target [Service] Type=oneshot ExecStart=/usr/local/bin/zswap-config.sh RemainAfterExit=true [Install] WantedBy=multi-user.target UNIT systemctl daemon-reload || true systemctl enable --now "${otimization_services[@]}" zswap-config.service || true if command -v udevadm &>/dev/null; then udevadm trigger --action=change --subsystem-match=power_supply; fi _ui_info "sucesso" "ZSWAP aplicado."
...
aplicar_zram() { _log "Aplicando ZRAM (Híbrido AC/Battery)"
...
#!/usr/bin/env bash CPU_CORES=$(nproc) if [[ -z "$CPU_CORES" ]]; then CPU_CORES=4; fi MAX_RETRY=10 count=0 while lsmod | grep -q "zram" && [ $count -lt $MAX_RETRY ]; do swapoff /dev/zram* 2>/dev/null || true echo 1 > /sys/block/zram0/reset 2>/dev/null || true echo 1 > /sys/block/zram1/reset 2>/dev/null || true modprobe -r zram 2>/dev/null || true sleep 1 ((count++)) done modprobe zram num_devices=2 2>/dev/null || true if command -v udevadm &>/dev/null; then udevadm settle; else sleep 3; fi

if [ -d "/sys/block/zram0" ]; then echo 1 > /sys/block/zram0/reset 2>/dev/null || true if command -v udevadm &>/dev/null; then udevadm settle; else sleep 0.5; fi echo lz4 > /sys/block/zram0/comp_algorithm 2>/dev/null || true echo "$CPU_CORES" > /sys/block/zram0/max_comp_streams 2>/dev/null || true echo zsmalloc > /sys/block/zram0/zpool 2>/dev/null || true echo 2G > /sys/block/zram0/disksize 2>/dev/null || true mkswap /dev/zram0 2>/dev/null || true swapon /dev/zram0 -p 3000 2>/dev/null || true fi

if [ -d "/sys/block/zram1" ]; then echo 1 > /sys/block/zram1/reset 2>/dev/null || true if command -v udevadm &>/dev/null; then udevadm settle; else sleep 0.5; fi echo zstd > /sys/block/zram1/comp_algorithm 2>/dev/null || true echo "$CPU_CORES" > /sys/block/zram1/max_comp_streams 2>/dev/null || true echo zsmalloc > /sys/block/zram1/zpool 2>/dev/null || true echo 4G > /sys/block/zram1/disksize 2>/dev/null || true mkswap /dev/zram1 2>/dev/null || true swapon /dev/zram1 -p 10 2>/dev/null || true fi

echo 1 > /sys/kernel/mm/page_idle/enable 2>/dev/null || true sysctl -w vm.fault_around_bytes=32 2>/dev/null || true echo "=== ZRAM STATUS ===" >> /var/log/turbodecky.log zramctl >> /var/log/turbodecky.log ZRAM_SCRIPT

chmod +x /usr/local/bin/zram-config.sh
cat <<UNIT > /etc/systemd/system/zram-config.service

[Unit] Description=ZRAM Dual Layer Setup Persistent After=local-fs.target [Service] Type=oneshot ExecStart=/usr/local/bin/zram-config.sh RemainAfterExit=true [Install] WantedBy=multi-user.target UNIT systemctl daemon-reload || true systemctl enable --now "${otimization_services[@]}" zram-config.service || true if command -v udevadm &>/dev/null; then udevadm trigger --action=change --subsystem-match=power_supply; fi _ui_info "sucesso" "ZRAM Dual Layer aplicado."
...
reverter_alteracoes() { _executar_reversao _ui_info "sucesso" "Reversão completa. Reinicie." }

--- NOVA FUNÇÃO: Remover kernel customizado e reinstalar linux-neptune ---

_restore_kernel_to_neptune() { _log "Iniciando restauração do kernel padrão (linux-neptune)"
...
main() { echo -e "\n=== Turbo Decky $versao ===" echo "Bem vindo ao Turbo Decky! Otimize seu Steam Deck para obter o melhor desempenho em jogos!" echo "Todas as otimizações são seguras e podem ser revertidas."
